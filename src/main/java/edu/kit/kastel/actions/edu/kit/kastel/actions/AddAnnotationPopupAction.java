...
public class AddAnnotationPopupAction extends AnAction {
  ...
  public void update(@NotNull AnActionEvent e) {
    ...
    e.getPresentation().setEnabledAndVisible(caret != null && AssessmentModeHandler.getInstance().getCurrentExerciseConfig().isPresent());
  }
  ...
  public void actionPerformed(@NotNull AnActionEvent e) {
    ...
    //if no exercise config is loaded, we cannot make annotations
    //if there is no caret we can not sensibly display a popup
    if (caret == null || AssessmentModeHandler.getInstance().getCurrentExerciseConfig().isEmpty()) {
      return;
    }

    //collect all mistake types that can be annotated
    List<IMistakeType> mistakeTypes = AssessmentModeHandler
            .getInstance()
    ...
    JBPopupFactory
            ...
            .createPopupChooserBuilder(
                    mistakeTypes
                            .stream()
                            .map(DisplayableMistakeType::new).toList()
            )
